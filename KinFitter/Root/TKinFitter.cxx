...
TKinFitter::TKinFitter() :
   TNamed("UnNamed", "UnTitled"), _A(1, 1), _AT(1, 1), _B(1, 1), _BT(1, 1), _V(1, 1), _Vinv(1, 1),
         _VB(1, 1), _VBinv(1, 1), _VA(1, 1), _VAinv(1, 1), _c(1, 1), _C11(1, 1), _C11T(1, 1), _C21(
               1, 1), _C21T(1, 1), _C22(1, 1), _C22T(1, 1), _C31(1, 1), _C31T(1, 1), _C32(1, 1),
         _C32T(1, 1), _C33(1, 1), _C33T(1, 1), _deltaA(1, 1), _deltaY(1, 1), _deltaAstar(1, 1),
         _deltaYstar(1, 1), _lambda(1, 1), _lambdaT(1, 1), _lambdaVFit(1, 1), _yaVFit(1, 1),
         _constraints(0), _measParticles(0), _unmeasParticles(0) {
...
TKinFitter::TKinFitter(const TString &name, const TString &title) :
   TNamed(name, title), _A(1, 1), _AT(1, 1), _B(1, 1), _BT(1, 1), _V(1, 1), _Vinv(1, 1), _VB(1, 1),
         _VBinv(1, 1), _VA(1, 1), _VAinv(1, 1), _c(1, 1), _C11(1, 1), _C11T(1, 1), _C21(1, 1),
         _C21T(1, 1), _C22(1, 1), _C22T(1, 1), _C31(1, 1), _C31T(1, 1), _C32(1, 1), _C32T(1, 1),
         _C33(1, 1), _C33T(1, 1), _deltaA(1, 1), _deltaY(1, 1), _deltaAstar(1, 1),
         _deltaYstar(1, 1), _lambda(1, 1), _lambdaT(1, 1), _lambdaVFit(1, 1), _yaVFit(1, 1),
         _constraints(0), _measParticles(0), _unmeasParticles(0) {
...
void TKinFitter::addMeasParticles(TAbsFitParticle* p1, TAbsFitParticle* p2, TAbsFitParticle* p3,
      TAbsFitParticle* p4, TAbsFitParticle* p5, TAbsFitParticle* p6, TAbsFitParticle* p7,
      TAbsFitParticle* p8, TAbsFitParticle* p9) {
...
Int_t TKinFitter::fit() {
   ...
   do {
      ...
      while (currF >= prevF) {
         nstep++;
         //if (nstep < 10) {cout <<nstep <<" : currF: "<< currF << "\t , prevF: " << prevF << endl;}
	 if (nstep == 10)
	 {
	     _status=-10;
	     break;
...
void TKinFitter::setCovMatrix(TMatrixD &V) {
   ...
   if ((V.GetNrows() != _nParB) || (V.GetNcols() != _nParB)) {
      std::cout << "\n" << "Covariance matrix of measured particles needs to be a " << _nParB
            << "x" << _nParB << " matrix.";
   } else {
      _V.ResizeTo(V);
      _V = V;
...
Bool_t TKinFitter::calcV() {
   ...
   // Check if matrix can be inverted at all
   if (_Vinv.Determinant() == 0) {
      cout << "!!! ERROR: Matrix _V is singular!!!" << endl;
      return false;
   }
   //try {
   _Vinv.Invert();
   //} catch (cms::Exception& e) {
   //  std::cout << "\n" << "Failed to invert covariance matrix V.";
   //}
...
Bool_t TKinFitter::calcVB() {
   ...
   _VB = _VBinv;
   // Check if matrix can be inverted at all
   if (_VB.Determinant() == 0) {
      cout << "!!! ERROR: Matrix _VB is singular!!!" << endl;
      return false;
   }
   //try {
   _VB.Invert();
...
Bool_t TKinFitter::calcVA() {
   ...
   _VAinv = _VA;
   // Check if matrix can be inverted at all
   if (_VA.Determinant() == 0) {
      cout << "!!! ERROR: Matrix _VA is singular!!!" << endl;
      return false;
   }
   //try {
   _VAinv.Invert();
...
void TKinFitter::print() {
   ...
   cout << "\n" << "\n";
   // Print status of fit
   cout << "Status: " << getStatusString() << "   F=" << getF() << "   S=" << getS() << "   N="
         << _nbIter << "   NDF=" << getNDF() << "\n";
   // Print measured particles
   cout << "measured particles: \n";
   Int_t parIndex = 0;
   for (unsigned int iP = 0; iP < _measParticles.size(); iP++) {
      TAbsFitParticle* particle = _measParticles[iP];
      Int_t nParP = particle->getNPar();
      ...
      cout << particle->getInfoString();
   }
   // Print unmeasured particles
   cout << "unmeasured particles: \n";
   parIndex = 0;
   for (unsigned int iP = 0; iP < _unmeasParticles.size(); iP++) {
      TAbsFitParticle* particle = _unmeasParticles[iP];
      Int_t nParP = particle->getNPar();
   ...
   cout << "constraints: \n";
   for (unsigned int indexConstr = 0; indexConstr < _constraints.size(); indexConstr++) {
      cout << _constraints[indexConstr]->getInfoString();
   }
   cout << "\n";
...
void TKinFitter::printMatrix(const TMatrixD &matrix, const TString name) {
   ...
   cout << setfill('=') << setw(sw) << "=  " << name << setw(sw) << left << "  =" << "\n"
         << setfill(' ') << right << "\n";

   cout << nRows << "x" << nCols << " matrix is as follows \n";
