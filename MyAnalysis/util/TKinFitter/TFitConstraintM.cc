...
TFitConstraintM::TFitConstraintM() :
   TAbsFitConstraint(), _ParList1(0), _ParList2(0), _TheMassConstraint(0) {

}

TFitConstraintM::TFitConstraintM(vector<TAbsFitParticle*>* ParList1,
      vector<TAbsFitParticle*>* ParList2, Double_t Mass, Double_t normalization) :
   TAbsFitConstraint(normalization), _ParList1(0), _ParList2(0) {
...
TFitConstraintM::TFitConstraintM(const TString &name, const TString &title,
      vector<TAbsFitParticle*>* ParList1, vector<TAbsFitParticle*>* ParList2, Double_t Mass,
      Double_t normalization) :
   TAbsFitConstraint(name, title, normalization), _ParList1(0), _ParList2(0) {
   // ParList1: Vector containing first list of constrained particles
   //           ( sum[ m_i ] - sum[ m_j ] - MassConstraint == 0 )
   // ParList2: Vector containing second list of constrained particles
   //           ( sum[ m_i ] - sum[ m_j ] - MassConstraint == 0 )
...
void TFitConstraintM::addParticles1(TAbsFitParticle* p1, TAbsFitParticle* p2, TAbsFitParticle* p3,
      TAbsFitParticle* p4, TAbsFitParticle* p5, TAbsFitParticle* p6, TAbsFitParticle* p7,
      TAbsFitParticle* p8, TAbsFitParticle* p9, TAbsFitParticle* p10) {
...
void TFitConstraintM::addParticles2(TAbsFitParticle* p1, TAbsFitParticle* p2, TAbsFitParticle* p3,
      TAbsFitParticle* p4, TAbsFitParticle* p5, TAbsFitParticle* p6, TAbsFitParticle* p7,
      TAbsFitParticle* p8, TAbsFitParticle* p9, TAbsFitParticle* p10) {
...
TMatrixD* TFitConstraintM::getDerivative(TAbsFitParticle* particle) {
   ...
   if (OnList(&_ParList1, particle)) {
      ...
      if (Pf.M() == 0.) {
         std::cout << "\n" << "Division by zero in " << IsA()->GetName() << " (named " << GetName()
               << ", titled " << GetTitle()
               << ") will lead to Inf in derivative matrix for particle " << particle->GetName()
               << ".";
      }
      Factor = 1. / Pf.M();
...
Bool_t TFitConstraintM::OnList(vector<TAbsFitParticle*>* List, TAbsFitParticle* particle) {
